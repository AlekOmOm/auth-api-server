...
test.describe("Auth System - Complete Registration & Authentication Tests", () => {
   ...
   test("should display auth system homepage", async ({ page }) => {
      await page.waitForSelector("#app > *", { timeout: 10000 }); // Wait for Svelte app to mount
      await expect(
   ...
   test("should navigate to registration page", async ({ page }) => {
      ...
      await expect(
         page.getByRole("heading", { name: "Create Account", level: 2 })
      ).toBeVisible();
   });
   ...
   test("should register client app user successfully", async ({ page }) => {
      ...
      await page.fill('input[name="email"]', "playwrightuser@example.com");
      await page.fill('input[name="password"]', "TestPassword123!");
   ...
   test("should logout successfully", async ({ page }) => {
      ...
      // Should redirect to login or home page without authentication
      const bodyText = await page.textContent("body");
      expect(
