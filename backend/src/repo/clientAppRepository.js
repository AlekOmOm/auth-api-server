...
import * as sessionRepo from "./repositories/authInternal/sessionRepository.js";
...
const getPool = async (schema = DEFAULT_SCHEMA) => {
   if (!pools[schema]) {
      try {
...
const createUser = async (schema = DEFAULT_SCHEMA, user) => {
   console.log("🗄️ [USER REPO] Creating user in schema:", schema);
   console.log("🗄️ [USER REPO] User data:", user);
