## Issue: Redirect from /register to /login (Playwright Test Observation)

**Date Observed**: May 26, 2025 (During Playwright Test Run)

**Behavior**: 
When the Trading-Sim application attempts to redirect a user to the Auth-System's registration page (`${authSystemUrl}/register?return_url=...`), the user is ultimately landing on the Auth-System's login page (`${authSystemUrl}/login?...`).

**Trading-Sim Code Verification**:
The `authService.redirectToRegister()` function in Trading-Sim correctly constructs the URL to point to `/register` on the Auth-System.

**Test Case**: `01-landing-page.spec.js` - `should navigate to auth system on get started click`
-   **Action**: Clicks "Get Started" button on Trading-Sim's landing page.
-   **Expected Trading-Sim Redirect**: To `${authSystemUrl}/register?return_url=...`
-   **Actual Landing URL in Browser (Observed in Playwright)**: `${authSystemUrl}/login?return_url=...`

**Suspected Cause (Auth-System Side)**:
The Auth-System might be internally redirecting requests from its `/register` endpoint to its `/login` endpoint under certain conditions. This could be due to session states, feature flags, or other internal logic within the Auth-System.

**Request**: 
Please investigate why a direct navigation or redirect to the Auth-System's `/register` path results in the user landing on the `/login` path. Is there a specific condition or configuration required to access the `/register` page directly?

---

## CRITICAL Issue: Auth-System Post-Login Redirect to `return_url` Failing
...
## CRITICAL Issue: Test User Credential Failures (API Error 401)
...
## NEW Issue: Post-Registration Login Does Not Redirect to `return_url`
